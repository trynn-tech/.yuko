...
class AnchorPatcher:
    ...
    def parse_blocks(self, raw_output: str, fallback_filepath: str = "") -> List[PatchBlock]:
        ...
        if matches:
            for match in matches:
                ...
                filepath = raw_path.strip() if raw_path else fallback_filepath
        ...
        extracted = self._extract_code_fault_tolerant(fallback_filepath, cleaned)
        if extracted.strip():
            blocks.append(
                PatchBlock(
    ...
    def locate_fuzzy_anchor(
        ...
        if norm_search in content:
            for i in range(len(content_lines) - window_size + 1):
                if "\n".join(content_lines[i : i + window_size]).strip() == norm_search:
    ...
    def wrap_symbolic_syntax(self, code: str, ast_facts: Dict[str, Any]) -> str:
        """
        Compresses vector relations and injects symbolic annotations ('@') 
        for custom reasoning DSL parsing and structural scoping.
        """
        funcs = ast_facts.get("functions", [])
        wrapped_lines = []
        for line in code.splitlines():
            trimmed = line.strip()
            if any(trimmed.startswith(f"def {fn}") or trimmed.startswith(f"function {fn}") for fn in funcs):
    ...
    def _extract_code_fault_tolerant(self, filepath: str, text: str) -> str:
        ...
        if fence_matches:
            ...
            for match in fence_matches:
                ...
                if len(candidate) > len(best_candidate) and self._validate_syntax(ext, candidate):
    ...
    def _validate_syntax(self, ext: str, code: str) -> bool:
        ...
        prose_phrases = [
            "here is", "this file", "breakdown of", "key components",
            "user's shell", "home manager is used", "sets up a user",
            "defines a user environment", "this nixos configuration"
        ]
        ...
        prose_line_count = sum(
            1 for line in lines
            if line.strip().endswith((".", ",", ";")) and not line.strip().startswith(("#", "{", "}", "[", "]", "mk", "environment", "home", "programs", "services"))
        )
    ...
    def extract_ast_facts(self, filepath: str, code: str) -> Dict[str, Any]:
        """Extracts functions, classes, imports, and complexity metrics using Python AST and Tree-Sitter grammars."""
        ext = Path(filepath).suffix.lower() if filepath else ""
        facts: Dict[str, Any] = {
            "functions": [],
            "classes": [],
        ...
        if HAS_TREE_SITTER and ext in LANGUAGES:
            try:
                parser = Parser(LANGUAGES[ext])
                tree = parser.parse(bytes(code, "utf-8"))
                func_types = {"function_definition", "async_function_definition", "function_item"}
                class_types = {"class_definition", "struct_specifier", "class_specifier"}
                import_types = {"import_statement", "import_from_statement", "preproc_include"}
                def traverse(node):
